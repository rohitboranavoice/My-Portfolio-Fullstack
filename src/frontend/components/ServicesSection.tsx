      ...
      <div className="container mx-auto px-6 max-w-screen-2xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {displayServices.map((service, index) => (
            ...
            >
              {/* Top Cover Image */}
              <Link href={`/service/${service.serviceId || service.id}`} className="block relative h-72 min-h-0 shrink-0 w-full overflow-hidden bg-[#FAFAFA] border-b border-gray-100">
                <img 
                  src={service.image} 
                  alt={service.title} 
                  className="absolute inset-0 w-full h-full object-contain transform group-hover:scale-105 transition-transform duration-700" 
                  loading="lazy" 
                />
