...
// The hardcoded data to seed
const servicesData = [
  {
    serviceId: "event-photography",
...
export async function GET() {
  ...
  try {
    ...
    // Optional: Clear existing services first to avoid duplicates if serviceId is unique
    await Service.deleteMany({});
    
    // Insert all services
    const results = await Service.insertMany(servicesData);
    
    return NextResponse.json({ 
      success: true, 
      count: results.length,
      message: "Database seeded successfully with default services."
    });
  } catch (error: any) {
